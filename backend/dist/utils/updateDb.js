...
const runUpdates = async () => {
    ...
    await supabaseClient_1.supabase.from('products').update({ image_url: 'https://images.unsplash.com/photo-1601925228008-54d428e4c55b?w=400&h=300&fit=crop' }).eq('name', 'Yoga Mat Premium');
    await supabaseClient_1.supabase.from('products').update({ image_url: 'https://images.unsplash.com/photo-1584100936595-c0654b55a2e2?w=400&h=300&fit=crop' }).eq('name', 'Linen Throw Pillow Set');
    await supabaseClient_1.supabase.from('products').update({ image_url: 'https://images.unsplash.com/photo-1615526675159-e248c3021d3f?w=400&h=300&fit=crop' }).eq('name', 'Wireless Charging Pad');
    ...
    const { error } = await supabaseClient_1.supabase.from('products').insert(newProducts);
    if (error) {
