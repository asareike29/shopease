...
const testDb = async () => {
    ...
    const { data: currentData } = await supabaseClient_1.supabase
        .from('products')
    ...
    for (const product of productsToUpsert) {
        const { data: existing } = await supabaseClient_1.supabase.from('products').select('id').eq('name', product.name).single();
        if (existing && existing.id) {
            await supabaseClient_1.supabase.from('products').update(product).eq('id', existing.id);
            console.log(`Updated ${product.name}`);
        }
        else {
            await supabaseClient_1.supabase.from('products').insert([product]);
            console.log(`Inserted ${product.name}`);
        }
    }
    console.log("\n=== STEP 6: VERIFY CATEGORY COUNTS ===");
    const { data: allProds } = await supabaseClient_1.supabase.from('products').select('name, category').order('name', { ascending: true });
    if (allProds) {
        const aggregate = allProds.reduce((acc, p) => {
            if (!acc[p.category])
                acc[p.category] = { total: 0, products: [] };
